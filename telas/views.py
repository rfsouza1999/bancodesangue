...
from bancodesangue.telas.models import Compartilhamento, Endereco, Pessoa
...
def dados_pessoa(request): ## FEITO
    if request.method == 'POST':
        ...
        Pessoa_Checkup = request.POST['Pessoa_Checkup']
        Pessoa_Doacoes = request.POST['Pessoa_Doacoes']
        dados_pessoa = Pessoa.objects.create(Pessoa_Cpf=Pessoa_Cpf, Pessoa_Nome=Pessoa_Nome, Pessoa_Telefone1=Pessoa_Telefone1, Pessoa_Telefone2=Pessoa_Telefone2, Pessoa_Peso=Pessoa_Peso,  Pessoa_Altura=Pessoa_Altura, Pessoa_TipoSanguineo=Pessoa_TipoSanguineo, Pessoa_Sexo=Pessoa_Sexo, Pessoa_DataNascimento=Pessoa_DataNascimento, Pessoa_Endereco=Pessoa_Endereco, Pessoa_Checkup=Pessoa_Checkup, Pessoa_Doacoes=Pessoa_Doacoes)
        dados_pessoa.save()
        return redirect('dados_pessoa')
    return render(request, 'dados_pessoa.html')
    
def endereco(request): ## FEITO
    if request.method == 'POST':
        Endereco_Id = request.POST['Endereco_Id']
        ...
        endereco = Endereco.objects.create(Endereco_Id=Endereco_Id, Endereco_Cep=Endereco_Cep, Endereco_Rua=Endereco_Rua, Endereco_Numero=Endereco_Numero, Endereco_Bairro=Endereco_Bairro, Endereco_Cidade=Endereco_Cidade, Endereco_Estado=Endereco_Estado, Endereco_Pais=Endereco_Pais, Endereco_Obs=Endereco_Obs)
        endereco.save()
